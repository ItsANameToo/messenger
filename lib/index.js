        ...
        const messages = {
            discord: {
                'vote': (address, username, balance, id) => {
                    return `⬆️ **${address}** voted for **${username}** with **${balance} ARK**. [Open transaction](http://arkexplorer.xyz/#/transaction/${id})`
                },
                'unvote': (address, username, balance, id) => {
                    return `⬇️ **${address}** unvoted **${username}** with **${balance} ARK**. [Open transaction](http://arkexplorer.xyz/#/transaction/${id})`
                },
                'missed': (username) => {
            ...
            slack: {
                'vote': (address, username, balance, id) => {
                    return `⬆️ *${address}* voted for *${username}* with *${balance} ARK*. <https://arkexplorer.xyz/#/transaction/${id}|Open transaction>`
                },
                'unvote': (address, username, balance, id) => {
                    return `⬇️ *${address}* unvoted *${username}* with *${balance} ARK*. <http://arkexplorer.xyz/#/transaction/${id}|Open transaction>`
                },
                'missed': (username) => {
                    return `*${username}* missed a block`
                }
            ...
            default: {
                'vote': (address, username, balance, id) => {
                    return `⬆️ ${address} voted for ${username} with ${balance} ARK. https://arkexplorer.xyz/#/transaction/${id}`
                },
                'unvote': (address, username, balance, id) => {
                    return `⬇️ ${address} unvoted ${username} with ${balance} ARK. http://arkexplorer.xyz/#/transaction/${id}`
                },
                'missed': (username) => {
                    return `${username} missed a block`
                }
